		...
		<div v-else-if="CalViewMode === VIEW_MODE_WEEK">    <!-- MODALITA' SETTIMANALE -->
			<table style="border-collapse: collapse; width: 100%;" border="1" class="date-grid-week">
				...
				<tbody class="dayEvents_Week">
					<tr>    <!--Eventi ed attivit� prive di orario: giornata completa-->
						<td style="width: 2%;">&nbsp;</td>
						<td style="width: 10%;" scope="col"
							class="date"
							v-for="dateDay in DateGrid"     
...
<script setup>
	...
	var CalDate = ref(dayjs(Today.value).startOf('month').toDate());   //Inizialmente consideriamo oggi come data del calendario; � il primo giorno del mese per vis. mensile, primo giorno della settimana per vis. settimanale, giorno per vis. giornaliera
	...
	const DateGrid = computed(() => {
		...
		if (CalViewMode.value == VIEW_MODE_MONTH){    //Visualizzazione mensile
			for (let i = 0; i < CalDataDayJs.weekday(); i++){                    //Giorni della settimana precedenti il primo del mese, da luned�
				const item = {}
				item.id = -i
				item.num = null
				item.date = null
	...
	function getExpiringActivities(date){    //attivit� con scadenza "date"
		const list = []
